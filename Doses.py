...
class Doses(object):
    ...
    def __init__(self, dose_field , time_on, time_gap = [0]):
        self.dose_field = dose_field # a list of dose_fields
        self.time_on = time_on #a list of times the beam is on
        self.time_gap = time_gap # a list of time gaps
        #assume that all the dose_field have the same dimentions 
        (self.x_dim, self.y_dim, self.z_dim) = self.dose_field[0].shape

        
    def get_dose_fields(self):
    ...
    def add_new_dose_field(self, dose):
        new_dose = dose.get_dose_fields()
